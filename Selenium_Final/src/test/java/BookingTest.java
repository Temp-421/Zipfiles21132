...
public class BookingTest {
    ...
    public void validateAttractionSearch() throws InterruptedException {
        ...
        List<WebElement> names = driver.findElements(By.xpath("//h3[@data-testid='card-title']"));
        List<WebElement> prices = driver.findElements(By.xpath("//div[contains(@class,'e7addce19e')]"));

        Assert.assertTrue(names.size() > 0, "No attractions found");

        for (int i = 0; i < names.size(); i++) {
            System.out.println(
                    names.get(i).getText() + " | Price: " + prices.get(i).getText()
            );
        }
    }
